...
I hit another formatting snag today that will require me to go back and find different refernce files. I believe I generally have the `ExcavatorDataPrepare.pl` script setup correctly, but the chromosome naming scheme needs to be consistent across the reference and sample files. This should be relatively easy to fix. I just need to find fasta, bed, and bam files that match.
...
In addtiion, I reran the pipeline with a new reference due to the chromosome annotation issue. This includes some BAMs from UCSC that are smaller in size.
...
I've was able to run the `ExcavatorPrepareData.pl` script with the followig parameters. I'm not entirely sure this worked based on the output, but it doesn't seem explicity that it didn't work either.
...
I've run the final script for the Excv2 pipeline to no avail. I'm at a bit of a loss and will likely transition to VarScan2 today unless I get a decent lead from reaching out the Excv2 authors.
...
I need to build a pileup file with `samtools`. I don't know how long that will take, so I'm letting that run on msi. Currently, I am planning to use the UCSC hg19 fasta refernce with BAMs from the 1kgp (NA10847, NA19131).
