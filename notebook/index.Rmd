...
Looking at the bam files I created, I noticed they're all 92B. This is probably an indication something went wrong in their creation. The bwa script seems to work fine on its own, so I imagine the issue is the bash script I created.

* samples: `*.fastq`; `~/ocpmi/bam/wes`
* output: `~/ocpmi/bam/wes`
* gene_ref: `hg38.fa`, via UCSC; `~/ocpmi/gene_ref`

I'm trying a new script I found on biostars that uses gnu-parallel.
...
ls *.fastq | parallel -j 2 'bwa mem -t 4 ~/ocpmi/gene_ref/hg38.fa {} | samtools sort -@4 -o {.}.bam'
```
