...
* samples: `*.fastq.gz` via ocpmi; `~/ocpmi/data/bam/wes1`
* job: `2463992.node1081`
* star_vars: `100` (overhang); `Standard` (SAM attr)
* output: `~/ocpmi/data/bam/wes1`
* gene_ref: `hg38.fa`, `hg38.gtf` via UCSC; `~/ocpmi/gene_ref`
...
I was able to get this job started with a sizable resource req on itasca having seen that there was a decent amount of idle nodes there. Hopefully things run smoothly this time around.
...
Didn't get a lot of concrete work done today. Something randomly changed the permissions for my working directory today. I'm at a complete loss for why that would have happened. I was able to fix it for my file permissions, but my ssh keys are still not working. I don't know if those things are related, but they are equally weird and occurred at the same time.

Outside of that, my primary focus right now is the genomic prep of the new wes data so we can begin pushing it through the excv2 pipeline.
...
ID Started    Active Age P Project Tags Description
-- ---------- ------ --- - ------- ---- --------------------------
76 2019-08-28 2h     2d  H lab     work align new wes1 fastq files
...
Created    Completed  Age P Project Description
---------- ---------- --- - ------- -----------------------------
2019-08-27 2019-08-28 1d  H lab     transfer new wes data to wes1
2019-08-27 2019-08-28 1d    lab     req new vm for lab notebooks
...
ID Active Age P Project Tags Description                                  Urg
-- ------ --- - ------- ---- -------------------------------------------- ------
76 22h    2d  H lab     work align new wes1 fastq files                     11.8
64        2w    lab     work look for differences in 1kgp comparison in     1.88
                             exv2 paper
65        2w    lab     work do search for best suited cnv controls         1.88
67        9d    lab     work create full_send.sh for varscan                1.85
70        7d    lab     work followup with andy re possible cnv controls    1.84
71        7d    lab     work check if im using the immortalized or          1.84
                             baseline 10847
72        6d    lab     work do mind dump. regear direction                 1.83
79        2d    lab     work add results of excv2 bwa bams to results       1.81
                             page
39        3mo L lab     work agenda                                         0.33
73        4d  L lab     work setup termux with umn vpn                    -0.178
...
* samples: `*.fastq.gz` via ocpmi; `~/ocpmi/data/bam/wes1/tmp`
* job: `2464105.node1081`
* star_vars: `100` (overhang); `Standard` (SAM attr)
* output: `~/ocpmi/data/bam/wes1`
* gene_ref: `hg38.fa`, `hg38.gtf` via UCSC; `~/ocpmi/gene_ref`
...
I think I finally got this down. A major issue seemed to be that I needed to call `readFilesIn` before `readFilesCommand` for the decompression method. This took an outrageous amount of time to figure out, but it seems good now. The only issue that remains is storage management. I'm working with ~2TB. I'll do things incrementally so I can clear out some space as I go, but I may have to deliver the output to scratch if worse comes to worse.
...
The latest error I'm getting from the seq alignment process is the result of a lack of disk space. Apparently, I've taken up a massive amount of space with these wes files and will need to figure out how to manage those effectively since they are so large.

### Summary

Basically the whole day was spent troubleshooting the alignment process. Almost there, but it's been a grip since I've progressed forward. Additionally, I was able to resolve a few standing issues with OIT today, most notably of which is the access issue with ale03. This will allow me to move forward with transitioning the ccgd.
...
ID Started    Active Age P Project Tags Description
-- ---------- ------ --- - ------- ---- --------------------------
76 2019-08-28 1d     3d  H lab     work align new wes1 fastq files
...
Created    Completed  Age P Project Description
---------- ---------- --- - ------- -----------------------------------
2019-08-14 2019-08-29 2w  L lab     clarify duo login issues with ale01
...
There is a period of about 6 days extending from *2019-08-30* through *2019-09-04* were a combination of travel and illness put me in a relatively disorganized position. I continually ran several processes over this time with poor documentation due to my circumstances. As a result, I only really have the bare bones in terms of detail for my efforts during this period. Any analysis that did not produce a relevant output will not be documented.

### Ambiguous notes - start

### STAR align: wes1 ocpmi

* samples: `*.fastq.gz` via ocpmi; `~/ocpmi/data/bam/wes1`
* job: `2464146.node1081`, `2464147.node1081`, `2464196.node1081`
* star_vars: `100` (overhang); `Standard` (SAM attr)
* output: `~/ocpmi/data/bam/wes1`
* gene_ref: `hg38.fa`, `hg38.gtf` via UCSC; `~/ocpmi/gene_ref`

I needed to make some modifications to the STAR script to allow for the processing of compressed files. Given the size of these fastq files, it was necessary to complete the alignment with out an uncompresed intermediate. The command below takes this into account using the `readFilesCommand`.
...
### STAR align: wes1 NA10847

* samples: `z_NA10847_SRR070531_*` via 1kgp; `/ocpmi/data/bam/1kgp/star_aligned`
* job: `2464295.node1081`
* star_vars: `100` (overhang); `Standard` (SAM attr)
* output: `/ocpmi/data/bam/1kgp/star_aligned`
* gene_ref: `hg38.fa`, `hg38.gtf` via UCSC; `~/ocpmi/gene_ref`

The bam output for these files was transfered to `/home/chris/ocpmi/data/bam/wes1` to be processsed with the ocpmi bams.

### Bam indexing

* samples: `*.bam`; `/ocpmi/data/bam/wes1`
* job: `2464336.node1081`
* output: `/ocpmi/data/bam/wes1`
...
### excv2 ocpmi full 50k

* name: `star_wes1_full5_50000_2019-09-04_1523`
* job: `16973019.mesabim1`
* samples: `z_NA10847_SRR070531` (control), `ocpmi *.bam` (expt) via ; `/ocpmi/data/bam/wes1`
* excv2_vars: `50000` (window); `pooling` (analysis mode)
* output: `/ocpmi/results/excavator2/star_wes1_full5_50000_2019-09-04_1523`
* gene_ref: `hg38.fa`, `hg38_sortmerge.bed` via UCSC; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2

### excv2 ocpmi full 40k

* name: `star_wes1_40_40000_2019-09-05_0049`
* job: `16979880.mesabim1`
* samples: `z_NA10847_SRR070531` (control), `ocpmi *.bam` (expt) via ; `/ocpmi/data/bam/wes1`
...
### excv2 ocpmi full 30k

* name: `star_wes1_30_30000_2019-09-05_0103`
* job: `16979884.mesabim1`
* samples: `z_NA10847_SRR070531` (control), `ocpmi *.bam` (expt) via ; `/ocpmi/data/bam/wes1`
* excv2_vars: `30000` (window); `pooling` (analysis mode)
* output: `/ocpmi/results/excavator2/star_wes1_30_30000_2019-09-05_0103`
* gene_ref: `hg38.fa`, `hg38_sortmerge.bed` via UCSC; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2

### excv2 ocpmi full 20k

* name: `star_wes1_20_20000_2019-09-05_0105`
* job: `16979883.mesabim1`
* samples: `z_NA10847_SRR070531` (control), `ocpmi *.bam` (expt) via ; `/ocpmi/data/bam/wes1`
* excv2_vars: `20000` (window); `pooling` (analysis mode)
* output: `/ocpmi/results/excavator2/star_wes1_20_20000_2019-09-05_0105`
* gene_ref: `hg38.fa`, `hg38_sortmerge.bed` via UCSC; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2

### excv2 ocpmi full 10k

* name: `star_wes1_10_10000_2019-09-05_0124`
* job: `16979886.mesabim1`
* samples: `z_NA10847_SRR070531` (control), `ocpmi *.bam` (expt) via ; `/ocpmi/data/bam/wes1`
* excv2_vars: `10000` (window); `pooling` (analysis mode)
* output: `/ocpmi/results/excavator2/star_wes1_10_10000_2019-09-05_0124`
* gene_ref: `hg38.fa`, `hg38_sortmerge.bed` via UCSC; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2

### vs2 mpileup ocpmi

* samples: `z_NA10847_SRR070531` (normal), `ocpmi *.bam` (tumor) via ; `/ocpmi/data/bam/wes1`
* job: `816789.labq`
* output: `/ocpmi/data/bam/wes1`
* gene_ref: `hg38.fa`, via UCSC; `~/ocpmi/gene_ref`

I modified the `mpileup` script to allow for the use of `parallel.`
...
parallel 'samtools mpileup -q 1 -f ~/ocpmi/data/gene_ref/ucsc/hg38/hg38.fa z_NA10847_SRR070531_Aligned.sortedByCoord.out.bam {} > {.}.mpileup' ::: *.bam
...
These samples will be transferred to `/scratch.global/tasta005` temporarily as I determine what to do regarding the space constraints they create.
...
ID Active Age  P Project Tags Description                                   Urg
-- ------ ---- - ------- ---- --------------------------------------------- ----
83 11s    39s    lab     work create mpileup for wes1 data vs2               5.8
63        3w     lab     work look for differences in 1kgp comparison in    1.92
                              exv2 paper
64        3w     lab     work do search for best suited cnv controls        1.92
66        2w     lab     work create full_send.sh for varscan               1.89
69        13d    lab     work followup with andy re possible cnv controls   1.87
70        13d    lab     work check if im using the immortalized or         1.87
                              baseline 10847
71        13d    lab     work do mind dump. regear direction                1.87
75        9d     lab     work add results of excv2 bwa bams to results page 1.85
76        7d     lab     work setup starrnotes site                         1.84
82        1min   lab     work add rmarkdown render element to figs_final     1.8
84        25s    lab     work run vs2 copynumber function                    1.8
39        3mo  L lab     work agenda                                        0.37
...
I've created a directory under the global scratch to store things in transition.

* `/scratch.global/tasta005`

### vs2 mpileup copynumber pilot

**With Parallel**

* samples: `z_NA10847_SRR070531` (control), `ocpmi *.bam` (expt) via ; `/ocpmi/data/bam/wes1/wes1_tmp`
* job: `x`
* output: `/ocpmi/data/bam/wes1/wes1_tmp`
* gene_ref: `hg38.fa`, via UCSC; `~/ocpmi/gene_ref`

```{r}
parallel 'samtools mpileup -q 1 -f ~/ocpmi/data/gene_ref/ucsc/hg38/hg38.fa z_NA10847_SRR070531_Aligned.sortedByCoord.out.bam {} | java -jar ~/ocpmi/methods/varscan2/VarScan.v2.4.4.jar copynumber {.} --mpileup 1' ::: *.bam
```

**Brute without Parallel**

* samples: `z_NA10847_SRR070531` (control), `ocpmi *.bam` (expt) via ; `/ocpmi/data/bam/wes1/wes1_tmp1`
* job: `x`
* output: `/ocpmi/data/bam/wes1/wes1_tmp1`
* gene_ref: `hg38.fa`, via UCSC; `~/ocpmi/gene_ref`

The brute force method follows the given structure in the vs2 man to provide validation to the parallel method.

### Summary

Major cleanup day. I think I finally got caught up on nearly a weeks worth of analysis that I did not actively record. I have developed some degree of results that I need to evaluate tomorrow from the excv2 pipeline. In addition, I have begun the movement of the same data through the vs2 pipeline.
...
ID Started    Active Age Project Tags Description
-- ---------- ------ --- ------- ---- ------------------------------------------
80 2019-09-05 6h     6h  lab     work create mpileup for wes1 data vs2
84 2019-09-05 1min   3h  lab     work run pilot vs2 with piped mpileup to
                                      copynumber commmand
...
Created    Completed  Age P Project Description
---------- ---------- --- - ------- --------------------------------------------
2019-08-24 2019-09-05 12d L lab     setup termux with umn vpn
2019-08-26 2019-09-05 10d H lab     align new wes1 fastq files
2019-08-26 2019-09-05 10d   lab     run excv2 on wes1 ocpmi files
2019-08-22 2019-09-05 2w    lab     check if im using the immortalized or
                                    baseline 10847
2019-08-22 2019-09-05 2w    lab     do mind dump. regear direction
...
I've created updated pbs scripts to execute the vs2 pipeline to take advantage of gnu parallel and manage the increased size of the mpileup output.

**vs2_mpileup_create.pbs**

* Important! the output dir for this script is `/scratch.global/` due to the massive size of the mpileup files.
...
parallel 'java -jar ~/ocpmi/methods/varscan2/VarScan.v2.4.4.jar copynumber {} {.} --mpileup 1' ::: *.mpileup
```
...
### vs2 wes1 ocpmi mpileup creation

* samples: `NA10847_SRR070531_Aligned` (control), `*.bam` (expt) via ocpmi; `~/ocpmi/data/bam/wes1`
* job: `816985.labq`
* output: `/scratch.global/tasta005`
* gene_ref: `hg38.fa`, via UCSC; `~/ocpmi/gene_ref`

### vs2 wes1 ocpmi copynumber

* samples: `*.mpileup` via ocpmi; `/scratch.global/tasta005`
* job: `817151.labq`
* output: `~/ocpmi/results/varscan2/wes1`
* gene_ref: `hg38.fa`, via UCSC; `~/ocpmi/gene_ref`

### vs2 wes1 ocpmi copyCaller

* samples: `*.copynumber` via ocpmi; `~/ocpmi/results/varscan2/wes1`
* job: `817163.labq`
* output: `~/ocpmi/results/varscan2/wes1`
* gene_ref: `hg38.fa`, via UCSC; `~/ocpmi/gene_ref`
...
ID Age P Project Tags Description                                           Urg
-- --- - ------- ---- ----------------------------------------------------- ----
62 3w    lab     work look for differences in 1kgp comparison in exv2 paper 1.94
63 3w    lab     work do search for best suited cnv controls                1.94
65 2w    lab     work create full_send.sh for varscan                       1.91
68 2w    lab     work followup with andy re possible cnv controls           1.89
72 11d   lab     work setup starrnotes site                                 1.86
78 3d    lab     work add rmarkdown render element to figs_final            1.82
79 3d    lab     work review figure params for excv2                        1.82
84 59s   lab     work run dnacopy_seg_plot.R on wes1 data                    1.8
85 40s   lab     work follow up re starrnotes access                         1.8
39 3mo L lab     work agenda                                                0.39
...
Ken reached out re the disk space needed at the `/swadm` dir on the ccgd server. Based on an issue we're seeing with a nightly dump file, we will need more space on the new server. I created a ticket with OIT to have this arranged.

<details>
  <summary>**From Ken re disk space**</summary>
```{r}
From: <abbot195@umn.edu>
Subject: Disk space
Date: September 8, 2019 at 1:40:39 PM CDT
To: <oialinux@umn.edu>
Cc: "'Timothy Starr'" <star0044@umn.edu>

Greetings. Could we get another 30 GB provisioned for /swadm on ccgd-starrlab.oit.umn.edu? One of the files we download and digest on a nightly basis has been updated, and it’s now so large that it’s causing an update process to flop.

-Ken
```
</details>

### vs2 dnacopy_seg_plot.R

I've created an updated version of the DNAcopy script necessary to apply segmentation and visualization to the vs2 copynumber output.
...
### vs2 wes1 ocpmi DNAcopy

I processed the vs2 copynumber output from *2019-09-07* with the above `dnacopy_seg_plot.R` script. The output seems to be skewed either all positive or all negative depending on the patient. vs2 specifically addresses this with an argument in the `copyCaller` function called either `--recenter-up` or `--recenter-down`. This is something I need to do, but I worry there needs to be greater statistical validation to the recentering adjustment than is offered in vs2's documentation. This is something I need to explore.
...
```{r}
ID Age  D P Project Tags Description                                    Urg
-- ---- - - ------- ---- ---------------------------------------------- ----
88 4min     lab     work select new 1kgp control                         9.8
89 4min D   lab     work align new 1kgp control                          4.8
90 4min D   lab     work index new 1kgp control                          4.8
87 5min     lab     work make sense of excv2 viz [2]                     2.7
81 1d       lab     work explore cnvtogenes [1]                         2.61
63 3w       lab     work create full_send.sh for varscan                1.92
68 13d      lab     work setup starrnotes site                          1.87
74 6d       lab     work add rmarkdown render element to figs_final     1.83
79 2d       lab     work follow up re starrnotes access                 1.81
80 1d       lab     work add notes from weekend analysis                1.81
82 22h      lab     work troubleshoot window size excv2                  1.8
83 22h      lab     work troubleshoot recenter problem vs2               1.8
39 3mo    L lab     work agenda                                          0.4
91 3min D   lab     work run excv2 full_send with new 1kgp control      -3.2
...
* I need to take some time to explore the validation of my excv2 results
* Looking at the results of my excv2 output, the vcf region call files seem to correlate with the visualization. This will probably serve to allow for gene level association.
* I need to determine the most accurate window size. As the window decreases, there seem to be a greater number of calls.
* next steps would include drawing association to gene level variation
* describe the excv2 viz in greater detail

Actions

* explore excv2 viz. what adjustments can be made to output? what does each part of the viz mean?
* run another round of excv2 with new control. use old control as variable sample.
* explore statistical merits of window size.

### BAM create NA19131

As part of the process of trying to validate the results of my excv2 output, I am going to take another 1kgp genome and replace the NA10847 control to see if we can produce the same results. First I need to align this sequence.

* samples: `z_NA19131_SRR070494_1.fastq.gz`, `z_NA19131_SRR070494_2.fastq.gz` via 1kgp; `~/ocpmi/data/bam/1kgp/star_aligned`
* job: `2473057.node1081`
* star_vars: `100` (overhang); `Standard` (SAM attr)
* output: `~/ocpmi/data/bam/wes/star_aligned`
* gene_ref: `hg38.fa`, `hg38.gtf` via UCSC; `~/ocpmi/gene_ref`

**BAM Index**
* samples: `z_NA19131_SRR070494_Aligned.sortedByCoord.out.bam` via 1kgp; `~/ocpmi/data/bam/1kgp/star_aligned`
* job: `2473061.node1081`
* output: `~/ocpmi/data/bam/wes/star_aligned`

### excv2 ocpmi full validate control

* name: `star_wes1_controlvalidate`
* job: `17114108.mesabim1`
* samples: `NA19131` (control), `*.bam` (expt) via ; `~/ocpmi/data/bam/wes1`
* excv2_vars: `20000` (window); `pooling` (analysis mode)
* output: `~/ocpmi/results/excavator2`
* gene_ref: `hg38.fa`, `hg38_sortmerge.bed` via UCSC; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2
...
I'm concerned about the statistical validation in determining a window size for our cnv analysis. Given the wide range of reported calls when varying the window size, we may be looking at an elevated false discovery rate or missed calls. I found a thread where two investigators discuss methods for determining window size and cite their own work in this area. Both reference the expectation that we would be working with a poisson distribution, but overdispersion in real world data complicates this.

The first post offers a package called `CNAnorm` in bioc which allows for calculation of reasonable window size based on the sum of chromosome/contig length. The other major factor to consider is read depth, which I need to confirm with Tim.

* https://www.biostars.org/p/17899/

* samples: `NA10847` (control), `NA19131` (expt) via ; `~/ocpmi/data/bam/1kgp/star_aligned/bam2window_test`
* job: `817718.labq`
* output: `~/ocpmi/data/bam/1kgp/star_aligned/bam2window_test`
* gene_ref: `hg38.fa`, via UCSC; `~/ocpmi/gene_ref`

### Summary

Meeting with Tim today, we discussed the nature of my excv2 output from *2019-09-04*. We made some sense of this data and presented a few other questions that need to be addressed. Most importantly, I need to be able to validate the window size I select in running this analysis. Depending on what we go with, it can have a notable impact on false positives and negatives. Additionally, we need to rule out the need for other (different) controls. I'm running the excv2 pipeline again with a different 1kgp genome to assess this.

#### Running

```{r}
ID Started    Active Age   D Project Tags Description
-- ---------- ------ ----- - ------- ---- --------------------------------------
88 2019-09-11 20min  20min   lab     work run bam2window to test for window size
87 2019-09-11 8s     3h    D lab     work run excv2 full_send with new 1kgp
                                          control
...
```{r}
Created    Completed  Age Project Due        Description
---------- ---------- --- ------- ---------- -----------------------------------
2019-09-10 2019-09-11 1d  lab     2019-09-11 send excv2 20k figs to tim
2019-09-05 2019-09-11 6d  lab                review figure params for excv2
2019-09-09 2019-09-11 2d  lab                run dnacopy_seg_plot.R on wes1 data
2019-09-11 2019-09-11 2h  lab                select new 1kgp control
2019-09-11 2019-09-11 2h  lab                align new 1kgp control
2019-09-11 2019-09-11 2h  lab                index new 1kgp control
...
ID Age  P Project Tags Description                                    Urg
-- ---- - ------- ---- ---------------------------------------------- ----
86 1d     lab     work make sense of excv2 viz [2]                    2.71
81 2d     lab     work explore cnvtogenes [1]                         2.61
63 3w     lab     work create full_send.sh for varscan                1.93
74 7d     lab     work add rmarkdown render element to figs_final     1.84
79 3d     lab     work follow up re starrnotes access                 1.82
80 3d     lab     work add notes from weekend analysis                1.82
82 2d     lab     work troubleshoot window size excv2                 1.81
83 2d     lab     work troubleshoot recenter problem vs2              1.81
87 1d     lab     work run bam2window to test for window size         1.81
88 1d     lab     work confirm read depth data on wes                 1.81
89 2h     lab     work create heatmap of cnv gene association          1.8
90 7min   lab     work update project directories in notebook          1.8
39 3mo  L lab     work agenda                                         0.41

13 tasks
```

### star_wes1_controlvalidate_20000_2019-09-11_2140 {.tabset .tabfade}

#### Discussion

* name: `star_wes1_controlvalidate_20000_2019-09-11_2140`
* samples: `NA19131_SRR070494` (control), `*.bam` (expt) via ocpmi and 1kgp; `/ocpmi/data/bam/wes1`
* excv2_vars: `20000` (window); `pooling` (analysis mode)
* output: `/ocpmi/results/excavator2/star_wes1_controlvalidate_20000_2019-09-11_2140`
* gene_ref: `hg38.fa`, `hg38_sortmerge.bed` via UCSC; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2

I ran the full_send of excv2 with a new control from 1kgp. Eyeballing the results, it looks as if there is maybe an 85% overlap in CNV sites across all samples. There is a good deal of similarity, but the variation does raise the question of whether our controls are valid. It's possible we will need to create a "control consortium" of sorts with multiple controls to identify consistent common denominator CNV sites.

### ccgd and starrnotes

I spent a good deal of time troubleshooting the setup of the oit hosted linux servers today. The documentation is somewhat limited, and the nature of their privs setup is not entirely clear. I went through what I know to be the apache setup process with no resulting web page. There is something in the system setup I am unaware of that is preventing apache from serving pages.
...
Created    Completed  Age Project Description
---------- ---------- --- ------- ------------------------------------------
2019-09-11 2019-09-12 1d  lab     run excv2 full_send with new 1kgp control
2019-09-05 2019-09-12 7d  lab     add rmarkdown render element to figs_final
2019-09-09 2019-09-12 3d  lab     follow up re starrnotes access
2019-09-09 2019-09-12 3d  lab     add notes from weekend analysis

4 tasks
```


I have a ticket open to see if the apache setup used in RHEL6 will work in the new server since there is an automated script. If not, I will need to work with OIT more closely to see what the issue is in getting the webserver running.
...
ID Age P Project Tags Description                                Urg
-- --- - ------- ---- ------------------------------------------ ----
82 1d    lab     work make sense of excv2 viz [2]                2.71
77 3d    lab     work explore cnvtogenes [1]                     2.62
62 3w    lab     work create full_send.sh for varscan            1.93
78 2d    lab     work troubleshoot window size excv2             1.81
79 2d    lab     work troubleshoot recenter problem vs2          1.81
83 1d    lab     work run bam2window to test for window size     1.81
84 1d    lab     work confirm read depth data on wes             1.81
85 22h   lab     work create heatmap of cnv gene association      1.8
86 20h   lab     work update project directories in notebook      1.8
39 3mo L lab     work agenda [1]                                 1.21

10 tasks
```

### excv2 agilent pbmc control

Andy was able to get back to me re a wes control today. Once I have access to these, I will align them and send them through the excv2 pipeline.

```{r}
Hi Chris-

My apologies that I lost track of this. Christy did identify a sample you can use for your copy number experiments with the OCPMI bulk Agilent exome DNAseq data.

These are the two fastq files.  PLEASE let Christy know ASAP if you also want BAM/BAI from our clinical germline pipeline for these.

/panfs/roc/groups/6/clinicalmdl/shared/data_release/miscellaneous/180308_A00223_0008_AH7YG3DMXX/BT_WES_NovaSeq_Val/BTWES_Feb_12_2018_1_S4_R1_001.fastq.gz
/panfs/roc/groups/6/clinicalmdl/shared/data_release/miscellaneous/180308_A00223_0008_AH7YG3DMXX/BT_WES_NovaSeq_Val/BTWES_Feb_12_2018_1_S4_R2_001.fastq.gz

Note that these are currently in our clinical space which you cannot access.  Christy is going to move them to scratch and update on how to access.  But PLEASE respond ASAP if you need the BAM/BAI so we only have to do this once.
...
I spent some time digging through the R libs for the excv2 package since there isn't any detail on the plot params in the documentation. I was able to discern the following associations re the plots:

* Red line = HSLM SegMean value
* Grey bars = HSLM Log2 ratio value
* Light Blue dots = HSLM Log2 ratio value Out
* Dark Blue dots = HSLM Log2 ratio value In
* Dark Green = FastCall -2 call
* Light Green = FastCall -1 call
* Dark Red = FastCall +2 call
* Light Red = FastCall +1 call

I've reached out to the authors of the excv2 package to confirm these values. The only variable I need further detail on is the SegMean value. I have a sense of what this could be, but I don't want to make assumptions. As a result, I've asked for clarification on this from the authors as well.

### Summary

I was able to develop a better understanding of our excv2 output today, which was a big step forward. I'll need to pass this on to Tim when I get a chance. I also need to spend some time making sense of the in vs out targets that the pipeline is distinguishing between. In other news, Andy was able to get back to me re a potential agilent control. Once they're able to give me access to that data, I should be able to send that through the excv2 pipeline.
...
Created    Completed  Age   Project Description
---------- ---------- ----- ------- --------------------------------------------
2019-09-13 2019-09-13 25min lab     contact excavator2 authors re plot variables
2019-09-11 2019-09-13 2d    lab     make sense of excv2 viz
                                      2019-09-11 what adjustments can be made to
                                    the viz output?
                                      2019-09-11 what does each part of the viz
                                    mean?
...
ID Age D P Project Tags Description                                       Urg
-- --- - - ------- ---- ------------------------------------------------- -----
87 2d  D   lab     work align agilent pbmc fastqs                          4.81
88 2d  D   lab     work index agilent pbmc bams                            4.81
77 6d      lab     work explore cnvtogenes [1]                             2.63
62 3w      lab     work create full_send.sh for varscan                    1.95
67 2w      lab     work setup starrnotes site                               1.9
78 5d      lab     work troubleshoot window size excv2                     1.83
79 5d      lab     work troubleshoot recenter problem vs2                  1.83
82 4d      lab     work run bam2window to test for window size             1.82
83 4d      lab     work confirm read depth data on wes                     1.82
84 3d      lab     work create heatmap of cnv gene association             1.82
85 3d      lab     work update project directories in notebook             1.82
90 2d      lab     work make sense of in vs out targets excv2              1.81
91 2d      lab     work store git credentials                              1.81
92 2d      lab     work merge working branch to master                     1.81
39 3mo   L lab     work agenda [2]                                         1.33
89 2d  D   lab     work run excv2 full_send with agilent pbmc control     -3.19
...
I received access to the samples from the Christy and Andy today. I'll then begin the process of running this through the excv2 pipeline. The samples are located at the directory below:

* /home/nelsona2/shared/for_chris

### PBMC star align

* samples: `*.fastq.gz` via ocpmi; `~/ocpmi/data/bam/pbmc_control`
* job: `2473653.node1081`
* star_vars: `100` (overhang); `Standard` (SAM attr)
* output: `~/ocpmi/data/bam/pbmc_control`
* gene_ref: `hg38.fa`, `hg38.gtf` via UCSC; `~/ocpmi/gene_ref`

### PBMC bam index

* samples: `*.bam` via ocpmi; `~/ocpmi/data/bam/pbmc_control`
* job: `17142908.mesabim1`
* output: `~/ocpmi/data/bam/pbmc_control`

### PBMC wes1 excv2 full_send

* name: `star_wes1_pbmc`
* job: `17145051.mesabim1`
* samples: `z_BTWES_Feb_12_2018` (control), `*.bam` (expt) via ; `~/ocpmi/data/bam/wes1`
* excv2_vars: `20000` (window); `pooling` (analysis mode)
* output: `~/ocpmi/results/excavator2/star_wes1_pbmc_20000_2019-09-16_1729`
* gene_ref: `hg38.fa`, `hg38_sortmerge.bed` via UCSC; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2
...
The experiment from yesterday failed to produce an output. An error shown in the logs indicates that the age of the bam index file is older than the bam itself. This may have occurred when I copied the index file before the bam. I'm going to rerun adjusting some parameters to see if I can achieve some results.
...
ID Active Age   P Project Tags Description                                Urg
-- ------ ----- - ------- ---- ------------------------------------------ ----
63 17min  17min   lab     work run troubleshoot excv2 on pbmc              5.8
52        8d      lab     work explore cnvtogenes [1]                     2.64
43        4w      lab     work create full_send.sh for varscan            1.96
48        2w      lab     work setup starrnotes site                      1.91
53        7d      lab     work troubleshoot window size excv2             1.84
54        7d      lab     work troubleshoot recenter problem vs2          1.84
56        6d      lab     work run bam2window to test for window size     1.83
57        5d      lab     work create heatmap of cnv gene association     1.83
58        4d      lab     work make sense of in vs out targets excv2      1.82
59        4d      lab     work store git credentials                      1.82
25        3mo   L lab     work agenda [4]                                 1.44
...
The modified experiement didn't work, so I am running the excv2 `full_send` with just my 1kgp and pbmc files.

### PBMC test

* name: `pbmc_test`
* job: `17168653.mesabim1`
* samples: `NA10847` (control), `BTWES` (expt) via ; `~/ocpmi/data/bam/pbmc_control/test`
* excv2_vars: `20000` (window); `pooling` (analysis mode)
* output: `~/ocpmi/results/excavator2/pbmc_test_20000_2019-09-18_1428`
* gene_ref: `hg38.fa`, `hg38_sortmerge.bed` via UCSC; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2

The above experiment produced an output, so I'm going to adjust the sample setup and see if I can generate an output.

### PBMC test2

* name: `pbmc_test2`
* job: `818658.labq`
* samples: `BTWES` (control), `NA10847, PM-25` (expt) via ; `~/ocpmi/data/bam/pbmc_control/test`
* excv2_vars: `20000` (window); `pooling` (analysis mode)
* output: `~/ocpmi/results/excavator2/pbmc_test2_20000_2019-09-18_1710`
* gene_ref: `hg38.fa`, `hg38_sortmerge.bed` via UCSC; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2
...
This is the inverse of `pbmc_test.` The pbmc bam can run as an experimental sample, so we need to rule out if there is an issue with the ocpmi bams when it is run as the control.

* name: `pbmc_test3`
* job: `818671.labq`
* samples: `BTWES` (control), `NA10847` (expt) via ; `~/ocpmi/data/bam/pbmc_control/test`
* excv2_vars: `20000` (window); `pooling` (analysis mode)
* output: `~/ocpmi/results/excavator2/pbmc_test3_20000_2019-09-18_1938`
* gene_ref: `hg38.fa`, `hg38_sortmerge.bed` via UCSC; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2

### Summary

I spend most of the day troubleshooting the issues with running the new pbmc control through the excv2 pipeline. The output from the pipeline is not very verbose, so it is difficult to know what the issue is. If `pbmc_test3` doesn't work, I won't have much to go on as far as why the ocpmi and pbmc files don't want to cooperate. I guess even if it does work, I won't have a good explanation.
...
ID Started    Active Age Project Tags Description
-- ---------- ------ --- ------- ---- ------------------------------
61 2019-09-18 2h     2h  lab     work run troubleshoot excv2 on pbmc
...
Created    Completed  Age Project Description
---------- ---------- --- ------- ----------------------------------------------
2019-09-13 2019-09-18 4d  lab     get agilent pbmc fastqs from christy
2019-09-13 2019-09-18 4d  lab     align agilent pbmc fastqs
2019-09-13 2019-09-18 4d  lab     index agilent pbmc bams
2019-09-13 2019-09-18 4d  lab     run excv2 full_send with agilent pbmc control
2019-09-12 2019-09-18 5d  lab     update project directories in notebook
2019-09-13 2019-09-18 4d  lab     store git credentials
2019-09-09 2019-09-18 8d  lab     explore cnvtogenes
                                    2019-09-09
                                  https://github.com/Jeltje/cnvtogenes
...
* name: `pbmc_windowtest0_50000_2019-09-22_0021`
* job: `819013.labq`
* samples: `BTWES pbmc` (control), `wes1 *.bam`, `NA10847` (expt) via ; `/ocpmi/data/bam/wes1`
* excv2_vars: `50000` (window); `pooling` (analysis mode)
* output: `/ocpmi/results/excavator2/pbmc_windowtest0_50000_2019-09-22_0021`
* gene_ref: `hg38.fa`, `hg38_sortmerge.bed` via UCSC; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2

pbmc_windowtest1

* name: `pbmc_windowtest1_40000_2019-09-22_0024`
* job: `819014.labq`
* samples: `BTWES pbmc` (control), `wes1 *.bam`, `NA10847` (expt) via ; `/ocpmi/data/bam/wes1`
* excv2_vars: `40000` (window); `pooling` (analysis mode)
* output: `/ocpmi/results/excavator2/pbmc_windowtest1_40000_2019-09-22_0024`
* gene_ref: `hg38.fa`, `hg38_sortmerge.bed` via UCSC; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2

pbmc_windowtest4

* name: `pbmc_windowtest4_10000_2019-09-22_0028`
  job: `819017.labq`
* samples: `BTWES pbmc` (control), `wes1 *.bam`, `NA10847` (expt) via ; `/ocpmi/data/bam/wes1`
* excv2_vars: `10000` (window); `pooling` (analysis mode)
* output: `/ocpmi/results/excavator2/pbmc_windowtest4_10000_2019-09-22_0028`
* gene_ref: `hg38.fa`, `hg38_sortmerge.bed` via UCSC; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2

pbmc_windowtest5

* name: `pbmc_windowtest5_5000_2019-09-22_0033`
* job: `819018.labq`
* samples: `BTWES pbmc` (control), `wes1 *.bam`, `NA10847` (expt) via ; `/ocpmi/data/bam/wes1`
* excv2_vars: `1000` (window); `pooling` (analysis mode)
* output: `/ocpmi/results/excavator2/pbmc_windowtest5_5000_2019-09-22_0033`
* gene_ref: `hg38.fa`, `hg38_sortmerge.bed` via UCSC; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2

pbmc_windowtest6

* name: `pbmc_windowtest6_1000_2019-09-22_0037`
* job: `17203412.mesabim1`
* samples: `BTWES pbmc` (control), `wes1 *.bam` (expt) via ; `/ocpmi/data/bam/wes1`
* excv2_vars: `1000` (window); `pooling` (analysis mode)
* output: `/ocpmi/results/excavator2/pbmc_windowtest6_1000_2019-09-22_0037`
* gene_ref: `hg38.fa`, `hg38_sortmerge.bed` via UCSC; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2
...
* job: `17203578.mesabim1`

pbmc_windowtest8

status: failed

* job: `17203636.mesabim1`
...
ID Active Age P Project Tags Description                                Urg
-- ------ --- - ------- ---- ------------------------------------------ ----
49 5d     5d    lab     work run troubleshoot excv2 on pbmc             5.83
35        5w    lab     work create full_send.sh for varscan            1.99
39        3w    lab     work setup starrnotes site                      1.94
43        13d   lab     work troubleshoot window size excv2             1.87
44        13d   lab     work troubleshoot recenter problem vs2          1.87
45        12d   lab     work run bam2window to test for window size     1.87
46        11d   lab     work create heatmap of cnv gene association     1.86
47        10d   lab     work make sense of in vs out targets excv2      1.85
52        4d    lab     work move failed excv2 to folder                1.82
24        4mo L lab     work agenda [4]                                 1.47
...
* name: `pbmc_windowtest3_30000_2019-09-23_1642`
* job: `819249.labq`
* samples: `BTWES pbmc` (control), `wes1 *.bam` (expt) via ; `/ocpmi/data/bam/wes1`
* excv2_vars: `30000` (window); `pooling` (analysis mode)
* output: `/ocpmi/results/excavator2/pbmc_windowtest3_30000_2019-09-23_1642`
* gene_ref: `hg38.fa`, `hg38_sortmerge.bed` via UCSC; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2
...
ID Age  P Project Tags   Description                                        Urg
-- ---- - ------- ------ -------------------------------------------------- ----
29 4w     lab     work   setup starrnotes site                              1.95
49 5min   lab     n work find favicon for project notebook                   1.9
32 2w     lab     work   troubleshoot window size excv2                     1.88
33 2w     lab     work   run bam2window to test for window size             1.88
34 13d    lab     work   create heatmap of cnv gene association             1.87
35 12d    lab     work   make sense of in vs out targets excv2              1.87
50 1min   lab     work   use vcftools to evaluate excv2 output               1.8
51 42s    lab     work   possibly run analysis of read depth on samples      1.8
52 7s     lab     work   use featurefinder to evaluate excv2 output          1.8
16 4mo  L lab     work   agenda [1]                                         1.28
...
Tim has asked me to graph the results of the excv2 pipeline so as to draw out gene identiy from our variation results. I'm going to create a combination of bash and R scripts to do so.

#### excv2_gene_association.sh

This script is the main executable which calls the other scripts. It sould be run from the `Results` directory of an excv2 output. The workhorse of this script is the bedtools function `intersect` which can draw overlapping gene regions from different files.
...
################################################################################
#
# This script is meant to be run from the 'Results' directory of the Excavator2
# output.
#
################################################################################
...
rm *.txt
sed -i -e 's/_Aligned.sortedByCoord.out_call_subset//g'

echo "Graphing output"
geneAssociationGraph.R
...
This is an Rscript called by `excv2_gene_association.sh`. It's central purpose is to do the etl work on the excv2 vcf output.
...
This is the last step in the gene association process. This takes the transformed intersect dataset and graphs it as a heatmap.
...
geneAssociationTable <- read.table("geneAssociationTable", header = T, sep = "\t")
allGenes <- read.table("~/ocpmi/data/gene_ref/ucsc/hg38/hg38.bed", as.is = T, sep = "\t")
allGenes <- data.frame(geneId = allGenes$V4)
#allGenes[, ] <- lapply(allGenes[, ], as.character)
df <- merge(geneAssociationTable, allGenes, by = "geneId")

cols <- c("-2" = "orangered", "-1" = "orange", "1" = "lightblue", "2" = "blue")

ggplot(df, aes(sampleId, geneId, fill = call)) +
    geom_tile() +
    scale_fill_manual(values = cols) +
    theme(axis.title.x = element_blank(), axis.text.x = element_text(angle = 60, hjust = 1), axis.text.y = element_blank())
...
ID Age   P Project Tags       Description                                   Urg
-- ----- - ------- ---------- --------------------------------------------- ----
24 4w      lab     work       setup starrnotes site                         1.99
37 5d      lab     phone work call agilent re wes control data              1.93
26 3w      lab     work       troubleshoot window size excv2                1.92
27 2w      lab     work       create heatmap of cnv gene association         1.9
28 2w      lab     work       make sense of in vs out targets excv2          1.9
36 5d      lab     work       possibly run analysis of read depth on        1.83
                              samples
40 5d      lab     work       troubleshoot sshfs speed over vpn             1.83
45 1d      lab     work       update bash script code for sed command       1.81
46 1d      lab     work       troubleshoot plot output of gene association  1.81
                              script
47 1d      lab     work       change heatmap colors to red and blue         1.81
55 16min   lab     work       run gene association graph on excv2 output     1.8
                              with gene id bed file
56 15min   lab     work       rerun excv2 pipeline with agilent v6 bedfile   1.8
57 9s      lab     work       get apache working for ccgd                    1.8
14 4mo   L lab     work       agenda                                        0.52
...
Tim and I discussed my heatmap output from yesterday. I need to evaluate a few things with this. Primarily, I need to associate the cnv data I have to actual gene IDs rather than just transcript IDs like are in the bed file I've been using. Additionally, it came to my attention that agilent has a bed file specifically for the targets we've been working with, so I will need to try rerun things and look for differences.

### excv2 w/ wes1 and agilent bed

Per my conversation with Tim, I am going to rerun the excv2 pipeline on the wes1 bams to see if we get different results with the agilent targets. Given how excv2 manages the targeting process, I actually do expect this will have an impact.

I downloaded the BED files for from Agilent's website for the target capture.

* (S07604715) (https://earray.chem.agilent.com/suredesign/search.htm)

* name: `wes1_agilent_covered_01`
* job: `820649.labq`
* samples: `BTWES` (control), `*.bam ocpmi` (expt) via ; `~/ocpmi/data/bam/wes1`
* excv2_vars: `20000` (window); `pooling` (analysis mode)
* output: `~/ocpmi/results/excavator2`
* gene_ref: `hg38.fa` via UCSC, `S07604715_Covered_excv2_formatted.bed` via Agilent; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2

### Gene association heatmap

The bed file I've used to run excv2 and do the output-bed intersect was generated with ensembl transcript IDs. I'm going to be running the excv2 pipeline with the agilent target bed files, which may return different results. Regardless, I need to convert the final output to gene IDs eventually, so I'm going to try that with my existing bedfile.

Here's a possible bioMart tutorial to get a geneID annotated bed file:

* https://www.r-bloggers.com/biomart-and-biomart/

### Summary

I started a new run for the excv2 pipeline with the updated agilent v6 targets bed file. Waiting to see the results there, but I would not be surprised if they carry a significant difference in read out.

Also, I need to do some digging to find the correct annotations for a heatmap figure. Currently, the ucsc browser to generate eveverything at the transcript level. I could work with that by using the intersect bedtools function to create a gene-specific subset of ranges, but it may be safer just to generate one from scratch. The BioMart package seems to be the best bet at the moment.
...
ID Started    Active Age Project Tags Description
-- ---------- ------ --- ------- ---- ------------------------------------------
55 2019-10-02 1h     2h  lab     work rerun excv2 pipeline with agilent v6
                                      bedfile
...
Created    Completed  Age Project Description
---------- ---------- --- ------- --------------------------------------
2019-09-12 2019-10-02 2w  lab     create heatmap of cnv gene association
...
ID Age P Project Tags       Description                                     Urg
-- --- - ------- ---------- ----------------------------------------------- ----
18 5w    lab     work       setup starrnotes site                           2.01
29 10d   lab     phone work call agilent re wes control data                1.95
20 3w    lab     work       troubleshoot window size excv2                  1.94
21 3w    lab     work       make sense of in vs out targets excv2           1.93
28 10d   lab     work       possibly run analysis of read depth on samples  1.85
33 6d    lab     work       update bash script code for sed command         1.83
34 6d    lab     work       change heatmap colors to red and blue           1.83
42 4d    lab     work       get apache working for ccgd                     1.82
43 4d    lab     work       bring symlinks of all scripts in /usr/bin to my 1.82
                            project folder
46 3d    lab     work       add notation to heatmap scripts                 1.82
12 4mo L lab     work       agenda                                          0.55
...
I was able to retrieve an ensembl geneID bed file from bioMart. I'll need to do some discordant read analysis to see if there are transcripts with varying calls within a gene.

* http://useast.ensembl.org/biomart/martview/057861e7c4ab1683182100fdff6668ee
...
ID Age P Project Tags Description                                           Urg
-- --- - ------- ---- ----------------------------------------------------- ----
18 5w    lab     work setup starrnotes site                                 2.02
20 3w    lab     work troubleshoot window size excv2                        1.95
21 3w    lab     work make sense of in vs out targets excv2                 1.93
28 12d   lab     work possibly run analysis of read depth on samples        1.87
32 7d    lab     work update bash script code for sed command               1.84
33 7d    lab     work change heatmap colors to red and blue                 1.84
41 6d    lab     work get apache working for ccgd                           1.83
42 6d    lab     work bring symlinks of all scripts in /usr/bin to my       1.83
                      project folder
45 4d    lab     work add notation to heatmap scripts                       1.82
54 1d    lab     work explore discordant pairing in wes data                1.81
58 14s   lab     work get heatmap clustering working                         1.8
12 4mo L lab     work agenda [1]                                            1.35
...
ID Active Age P Project Tags Description                                    Urg
-- ------ --- - ------- ---- ---------------------------------------------- ----
57 3h     18h   lab     work run exc2 at different window ranges with        5.8
                             agilent bed
18        5w    lab     work setup starrnotes site                          2.02
20        4w    lab     work troubleshoot window size excv2                 1.95
21        3w    lab     work make sense of in vs out targets excv2          1.94
28        12d   lab     work possibly run analysis of read depth on samples 1.87
32        8d    lab     work update bash script code for sed command        1.84
40        6d    lab     work get apache working for ccgd                    1.83
41        6d    lab     work bring symlinks of all scripts in /usr/bin to   1.83
                             my project folder
44        5d    lab     work add notation to heatmap scripts                1.83
53        2d    lab     work explore discordant pairing in wes data         1.81
54        21h   lab     work get heatmap clustering working                  1.8
56        18h   lab     work look for discordant values in gene association  1.8
                             table
12        4mo L lab     work agenda [1]                                     1.36

13 tasks
```

### Gene association process

I've made notable updates to the several gene association heatmap scripts I've created. These 4 scripts should now run in concert for any excv2 output. I've also figured out the clustering method by using the `pheatmap` package.
...
ggsave("unclustered_plot.png", plot = g)
```
...
pheatmap(mat_unmelt,
         ...
         filename = "clustered_plot.png",
         main = args[2])
```

### wes1_agilent_covered_02_50000_2019-10-09_1332

* name: `wes1_agilent_covered_02_50000_2019-10-09_1332`
* job: `821837.labq`
* samples: `BTWES` (control), `*.bam ocpmi` (expt) via ; `/ocpmi/data/bam/wes1`
* excv2_vars: `50000` (window); `pooling` (analysis mode)
* output: `/ocpmi/results/excavator2/wes1_agilent_covered_02_50000_2019-10-09_1332`
* gene_ref: `hg38.fa`, `S07604715_Covered_excv2_formatted.bed` via Agilent; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2

### wes1_agilent_covered_03_40000_2019-10-09_1333

* name: `wes1_agilent_covered_03_40000_2019-10-09_1333`
* job: `821838.labq`
* samples: `BTWES` (control), `*.bam ocpmi` (expt) via ; `/ocpmi/data/bam/wes1`
* excv2_vars: `40000` (window); `pooling` (analysis mode)
* output: `/ocpmi/results/excavator2/wes1_agilent_covered_03_40000_2019-10-09_1333`
* gene_ref: `hg38.fa`, `S07604715_Covered_excv2_formatted.bed` via Agilent; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2

### wes1_agilent_covered_04_30000_2019-10-09_133

* name: `wes1_agilent_covered_04_30000_2019-10-09_1336`
* job: `821841.labq`
* samples: `BTWES` (control), `*.bam ocpmi` (expt) via ; `/ocpmi/data/bam/wes1`
* excv2_vars: `30000` (window); `pooling` (analysis mode)
* output: `/ocpmi/results/excavator2/wes1_agilent_covered_04_30000_2019-10-09_1336`
* gene_ref: `hg38.fa`, `S07604715_Covered_excv2_formatted.bed` via Agilent; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2

### wes1_agilent_covered_05_20000_2019-10-09_13

* name: `wes1_agilent_covered_05_20000_2019-10-09_1338`
* job: `821843.labq`
* samples: `BTWES` (control), `*.bam ocpmi` (expt) via ; `/ocpmi/data/bam/wes1`
* excv2_vars: `20000` (window); `pooling` (analysis mode)
* output: `/ocpmi/results/excavator2/wes1_agilent_covered_05_20000_2019-10-09_1338`
* gene_ref: `hg38.fa`, `S07604715_Covered_excv2_formatted.bed` via Agilent; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2

### wes1_agilent_covered_06_10000_2019-10-09_1

* name: `wes1_agilent_covered_06_10000_2019-10-09_1340`
* job: `821846.labq`
* samples: `BTWES` (control), `*.bam ocpmi` (expt) via ; `/ocpmi/data/bam/wes1`
* excv2_vars: `10000` (window); `pooling` (analysis mode)
* output: `/ocpmi/results/excavator2/wes1_agilent_covered_06_10000_2019-10-09_1340`
* gene_ref: `hg38.fa`, `S07604715_Covered_excv2_formatted.bed` via Agilent; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2

### wes1_agilent_covered_07_5000_2019-10-09_1

* name: `wes1_agilent_covered_07_5000_2019-10-09_1343`
* job: `17685360.mesabim1`
* samples: `BTWES` (control), `*.bam ocpmi` (expt) via ; `/ocpmi/data/bam/wes1`
* excv2_vars: `5000` (window); `pooling` (analysis mode)
* output: `/ocpmi/results/excavator2/wes1_agilent_covered_07_5000_2019-10-09_1343`
* gene_ref: `hg38.fa`, `S07604715_Covered_excv2_formatted.bed` via Agilent; `~/ocpmi/gene_ref`
* targets: `GCA_000001405.15_GRCh38.bw` via excv2

### wes1_agilent_covered_08_1000_2019-10-09_1345

* name: `wes1_agilent_covered_08_1000_2019-10-09_1345`
* job: `17685388.mesabim1`
* samples: `BTWES` (control), `*.bam ocpmi` (expt) via ; `/ocpmi/data/bam/wes1`
* excv2_vars: `5000` (window); `pooling` (analysis mode)
* output: `/ocpmi/results/excavator2/`
* gene_ref: `hg38.fa`, `S07604715_Covered_excv2_formatted.bed` via Agilent; `~/ocpmi/gene_ref`
...
### wes1_agilent_covered_09_500_2019-10-09_1347

* name: `wes1_agilent_covered_09_500_2019-10-09_1347`
* job: `17685418.mesabim1`
* samples: `BTWES` (control), `*.bam ocpmi` (expt) via ; `/ocpmi/data/bam/wes1`
* excv2_vars: `5000` (window); `pooling` (analysis mode)
* output: `/ocpmi/results/excavator2/`
* gene_ref: `hg38.fa`, `S07604715_Covered_excv2_formatted.bed` via Agilent; `~/ocpmi/gene_ref`
...
ID Age P Project Tags Description                                           Urg
-- --- - ------- ---- ----------------------------------------------------- ----
18 6w    lab     work setup starrnotes site                                 2.03
20 4w    lab     work troubleshoot window size excv2                        1.96
21 3w    lab     work make sense of in vs out targets excv2                 1.94
28 13d   lab     work possibly run analysis of read depth on samples        1.87
32 9d    lab     work update bash script code for sed command               1.85
39 8d    lab     work get apache working for ccgd                           1.84
40 7d    lab     work bring symlinks of all scripts in /usr/bin to my       1.84
                      project folder
51 3d    lab     work explore discordant pairing in wes data                1.82
53 1d    lab     work make git commits                                      1.81
54 1d    lab     work add notes about excv2 runs                            1.81
12 4mo L lab     work agenda [1]                                            1.36
...
We discussed the output I've generated through the gene association approach. The figure is generally looking good with how the data is clustering and showing notable grouping among both genes and patients.

One issue I raised was that there was some difficulty in finding a "clean" gene ID bed file. The dataset I had to filter the output probably contained a good deal of overlapping or generallly junk genes. My goal was to strive for a comprehensive list, but this was only really achievable through datasets that had a notable ambiguity (e.g. conveting a transcript ID list to gene ID). Tim gave me a list of genes he had created that was subsetted across genes that are all present in the RefSeq, CCDS lists as well as having a HUGO ID. I'm going to run the heatmap creation with this list and see what we end up with. In addition, we agreed that I should also do some subsetting by patient typing with the clustering results I see.

Typing (OCPMI spreadsheet):

* histotype
* molecular subtype
* chemoresistance

Tim also pointed me to the work of klaus pautel of germany. He presented somework on wes cnv methods at a conference in San Diego that Tim thought might be helpful.

### heatmap

I've updated the heatmap per the conversation I've had with Tim. There doesn't seem to be any sort of pattern formation around any of our patient parameters, which is odd. I'm going to explore the nature of the clustering method to see if there is a more appropriate way to cluster the results since I expect there to be at least some correlation in the cnv profile accross patient categorization.
...
Created    Completed  Age   Project Description
---------- ---------- ----- ------- --------------------------------------------
2019-10-04 2019-10-10 6d    lab     add notation to heatmap scripts
2019-10-08 2019-10-10 2d    lab     get heatmap clustering working
2019-10-08 2019-10-10 1d    lab     run exc2 at different window ranges with
                                    agilent bed
2019-10-09 2019-10-10 1d    lab     make git commits
2019-10-10 2019-10-10 56min lab     add subtyping color bars to wes heatmap
...
I spoke with Jun re his work on CNV with the ocpmi data. He has run inferCNV with pt59 and has sent me reference to some of his scripts. I can take a look at implementing this across all single cell samples.

#### From Jun

Hi Chris,

I put inferCNV on MSI public directory.

To load it,
1) $ module load JAGS/4.3.0-gcc7.2.0
2) $ R
   > library('infercnv', lib.loc='/home/starrt2/public/infercnv/library')

Example script and input files I used (msi37) are:
/home/starrt2/public/infercnv/msi37/infercnv.R
/home/starrt2/public/infercnv/msi37/msi37_epi_nonlog.rds (just the count matrix)
/home/starrt2/public/infercnv/msi37/gene_ordering_file.txt (grch38 gene locations)
/home/starrt2/public/infercnv/msi37/sample_annotation_file.txt (barcode vs. cluster IDs)

It takes several hours so would have to be submitted to a queue. The resulting figures etc are not optimal (chromosomes not ordered etc) but the output files are there, so I think we would need to generate our own figures from them. I can help with that as well.

If you get to try on other samples, I would be interested in knowing how it compares with exome seq results.
...
I was able to get the apache server running with a test index file. Here are the key points of the setup to get the base server running.

* Create `/swadm/var/www/html` to host site files.
* Set permissions of `/html`.
* Edit the `/etc/httpd/conf/httpd.conf` file to update the `DocumentRoot`.
* The `ServerRoot` can remain pointing at the original system site under the `/etc` directory.
* The `DocumentRoot` needs to be updated to the `/swadm` directory. Along with the `<Directory>` variable.
