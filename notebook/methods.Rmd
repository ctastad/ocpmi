...
I'm using `sshfs` to mount my MSI home folder on my local machine to make it more readily accessible. I already have ssh keys exchanged with MSI, so I can create an executable auto mount bash script. This is located at `/home/chris/starr_lab/automnt_msi` on my local machine.
...
I've run into the case where a server disconnect causes a hang any time I navigate to this mount point. To reset the mount point use the fusermount command to force the unmount first. I've also created an executable for this, `umount_msi`, in the same directory.

```{r eval=F}
#!/bin/bash

fusermount -u msi_mnt/tasta005
fusermount -u msi_mnt/starrt2
...
I am currently trying to get a wes pipeline to work using the following two bam sets from the 1kgp
