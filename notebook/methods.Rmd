...
│   ├── single_cell
│   └── wes
└── results # output for any code and pipelines
    ├── excavator2
    ├── seurat
    └── varscan2
...
```{r eval=T}
htmltools::includeHTML("~/ocpmi/methods/seurat/seurat_tutorial/pbmc3k_tutorial.html")
```
...
* pilot: `PM-2-T17-0307-3_S9_R1_001.fastq`, `PM-15-T17-0910-1_S2_R1_001.fastq`; `~/ocpmi/bam/wes/pilot`
...
* samples: `NA10847`, `NA19131` via 1kgp phase3
* output: `~/ocpmi/methods/EXCAVATOR2_Package_v1.1.2/data/output/analysis`
...
*&ast;.bed `sort` (bedtools) &rarr; [SourceTarget.txt (&ast;.bw + gene_ref) + &ast;.bed] `TargetPerla.pl` &rarr; [ExpFilePrep.txt (BAMs) + targets] `EXCAVATORDataPrepare.pl` &rarr; [ExpFileAn.txt (output + controls) + targets `EXCAVATORDataAnalysis.pl` &rarr; cnv output*

Condensed

*&ast;.bed `sort` (bedtools) &rarr; SourceTarget.txt `TargetPerla.pl` &rarr; ExpFilePrep.txt `EXCAVATORDataPrepare.pl` &rarr; ExpFileAn.txt `EXCAVATORDataAnalysis.pl` &rarr; cnv output*
