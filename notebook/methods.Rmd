...
I've run into the case where a server disconnect causes a hang any time I navigate to this mount point. To reset the mount point use the fusermount command to force the unmount first. I've also created an exacutable for this, `umount_msi`, in the same directory.
...
The default source file is `SourceTarget.txt` that is placed in the main program folder. `SourceTarget.txt` is a space delimitated file that contains the absolute paths to 1) a `bigWig file (.bw)` for the calculations of mappability and 2) to the reference genome sequence in .fasta format for GC-content calculations.
