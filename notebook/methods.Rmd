...
<details>
  <summary>**from: Ying re single cell location 9/26/19**</summary>
Hi, Chris,

/home/starrt2/shared/cellranger_v3_results

There is a file names “fname.txt” where you can find the data_release folder of UMGC raw reads.
...
* dir: `/home/starrt2/shared/cellranger_v3_results`
...
**Sequencing data (from Tim)**

Library was generated using Agilent's SureSelectXT Human All Exon V6 +COSMIC Catalog #'s 5190-9307 5190-9308 5190-9309.

* https://www.agilent.com/cs/library/datasheets/public/SureSelect%20V6%20DataSheet%205991-5572EN.pdf

I downloaded the BED files for from Agilent's website for the target capture. I downloaded by the BED files for Hg19 (GRCh37) and Hg38 (GRCh38). Christy Henzler's analysis used Hg19 (GRCh37) I'm pretty sure.

* (S07604715) (https://earray.chem.agilent.com/suredesign/search.htm)

Sequencing was done on two lanes of an Illumina NovaSeq S2 with 151 bp paired end reads. (see Nelson3_Project_003_NovaSeq_Summary.xlsx)

* Run name: 181103_A00223_0048_AHGMHLDMXX-analysis
* Total read-pairs = 4,462,035,977
* Mean read-pairs per sample = 139,438,624
...
│   └── star_aligned
├── pbmc_control # agilent v6 control from andy
│   ├── BTWES_Feb_12_2018__STARtmp
│   └── test
...
Also, job queue resource parameters
...
```{r}
varscan2/
├── dnacopy_seg_plot.R # script to execute DNAcopy: CBS algo and visualization
├── full_send.sh
├── test.R
└── VarScan.v2.4.4.jar
```
...
<details>
  <summary>**from: Andy 9/13/19 re excv2 agilent PBMC control**</summary>
...
```{r}
EXCAVATOR2_Package_v1.1.2/
├── config_files # source of configs from past runs
├── data
│   ├── centromere
...
├── full_send.sh # bash script to execute full excv2 pipeline
├── full_send.sh_old
├── full_send_vars.txt # config to define variables in full_send.sh
├── lib
│   ├── bash
│   ├── F77
│   ├── OtherLibrary
...
I've expanded it quite a bit to make it effectively pump and dump for the markdown output needed to go to the `results` page. It needs to be run from the `Plots` results directory.
...
### Gene Association Figs

The 4 scripts below work in concert to generate 2 heatmap figures. The first is the unclustered output of calls by ensembl geneID by patient. The second is the same dataset in a clustered heatmap created using the `pheatmap` package.

The `excv2_gene_association.sh` script should be run from the heatmap dir (`~/ocpmi/methods/EXCAVATOR2_Package_v1.1.2/heatmap`) with 2 arguments:

1. The excv2 results directory name (not the full path)
2. The desired title for the figure

Also, given the resource demand of `hclust`, this full process requires ~48GB of ram. Ran without `geneAssociationGraph_clust.R` can be done on any machine with basic resources.
...
<details>
  <summary>**from: Jun re implementation of SC inferCNV**</summary>
Hi Chris,
...
The new address has been approved, and my access has been cleared.

* address: `hst-ccgd-prd-web.oit.umn.edu`
...
Need to setup ssh keys for both general chef role (x500) and swadm user.

R was installed with the following tutorial
...
I've decided to effectively scrap the existing website. This may sound like a fools errand, but my investigation of the existing layout of the current site and database present a couple problems. As I have only attempted to transition the application, I have encountered some nuances that could take an undetermined amount of time to iron out. The layout of this thing is immense and complex. Also, there are a variety of frameworks that were chosen in the original setup which will certinaly present compatibility issues down the line. Given the effort already going into the migration. I feel it best to go the route of building this thing in R and shiny.

#### Version control

I've setup a git to track development of this project that is separate from the ocpmi project. It's located at the link below.

https://github.umn.edu/tasta005/ccgd.git

#### Shiny deployment

My plan is to deploy the shiny application to the public shinyapps.io hosting platform. This is done with the `rsconnect` package.

* docs: https://shiny.rstudio.com/articles/shinyapps.html
* platform: https://www.shinyapps.io/

The account information can be retrieved from the shinyapps.io site.
